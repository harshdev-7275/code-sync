...
const EditStudent = ({singleStudent, setSingleStudent, setEditStudentModel}) => {
  ...
  return (
    ...
    <form onSubmit={updateTeacherHandler} className='flex flex-col gap-5 items-center relative'>
      ...
      <input type="text" value={singleStudent?.id} className='outline-none border-2 border-white bg-[#3d3d3d] px-3 py-1 rounded' />
        </div>
        <div className='flex flex-col w-[400px] gap-2'>
    ...
    <button onClick={() => setEditStudentModel(false)} className='absolute top-2 right-2 text-xl bg-red-500 py-1 px-3 rounded-md hover:scale-95 transition-transform'>X</button>
    </form>
</div>
  )
}
